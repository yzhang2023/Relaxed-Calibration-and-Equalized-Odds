...
class Model(namedtuple('Model', 'pred label')):
    ...
    def eq_odds(self, othr, EQ_limit, Calib_limit, mix_rates=None):
        has_mix_rates = not (mix_rates is None)
        if not has_mix_rates:
            mix_rates = self.eq_odds_optimal_mix_rates(othr, EQ_limit, Calib_limit)
    ...
    def eq_odds_optimal_mix_rates(self, othr, EQ_lim, Calib_lim):
        ...
        spn_given_p = (sn2n * (sflip * sm_fn).mean() + sn2p * (sconst * sm_fn).mean()) / sbr + \
                      (sp2n * (sconst * sm_tp).mean() + sp2p * (sflip * sm_tp).mean()) / sbr

        spp_given_n = (sp2n * (sflip * sm_fp).mean() + sp2p * (sconst * sm_fp).mean()) / (1 - sbr) + \
                      (sn2p * (sflip * sm_tn).mean() + sn2n * (sconst * sm_tn).mean()) / (1 - sbr)

        opn_given_p = (on2n * (oflip * om_fn).mean() + on2p * (oconst * om_fn).mean()) / obr + \
                      (op2n * (oconst * om_tp).mean() + op2p * (oflip * om_tp).mean()) / obr

        opp_given_n = (op2n * (oflip * om_fp).mean() + op2p * (oconst * om_fp).mean()) / (1 - obr) + \
                      (on2p * (oflip * om_tn).mean() + on2n * (oconst * om_tn).mean()) / (1 - obr)

        if ((sbr-1) * self.fp_cost() + sbr * self.fn_cost()) >= 0:
            scal = ((sbr-1) * spp_given_n + sbr * spn_given_p)
        else:
            scal = -((sbr-1) * spp_given_n + sbr * spn_given_p)

        if ((obr-1) * othr.fp_cost() + obr * othr.fn_cost()) >= 0:
            ocal = ((obr-1) * opp_given_n + obr * opn_given_p)
        else:
            ocal = -((obr-1) * opp_given_n + obr * opn_given_p)

        N_Calib_lim = -Calib_lim
        ...
        constraints = [
            ...
            spp_given_n - opp_given_n <= EQ_lim,
            spn_given_p - opn_given_p <= EQ_lim,
        ...
        prob.solve()
        print("scal",scal.value)
        print("ocal",ocal.value)
        print("sfpr",spp_given_n.value)
        print("sfnr",spn_given_p.value)
        print("ofpr",opp_given_n.value)
        print("ofnr",opn_given_p.value)
        print("obase",obr)
...
if __name__ == '__main__':
    ...
    ```
    python eq_odds.py <path_to_model_predictions.csv> <Number for limit of Equalized Odds> <Number for limit of Calibration>
    ```

    Both the limits are numbers from 0 - 1, generally both below 0.1 would be ideal limits
    ...
    ```
    python eq_odds.py data/income.csv 0.06 0.08
    python eq_odds.py data/health.csv 0.03 0.05
    python eq_odds.py data/criminal_recidivism.csv 0.01 0.05
    ```
    """
    import pandas as pd
    import sys
    ...
    #Get the Equalized Odds and calibration limits
    EQ_limit = sys.argv[2]
    Calib_limit = float(sys.argv[3])
    ...
    _, _, mix_rates = Model.eq_odds(group_0_val_model, group_1_val_model, EQ_limit, Calib_limit)
